...
export default function ResetPassword() {
  ...
  async function submit(e) {
    e.preventDefault();
    setErr(null);
    if (password.length < 6) { setErr("La contraseÃ±a debe tener al menos 6 caracteres."); return; }
    if (password !== confirm) { setErr("Las contraseÃ±as no coinciden."); return; }
    setBusy(true);
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: `linear-gradient(135deg, ${C.abyss}, ${C.ocean})`, padding: 20 }}>
      <div style={{ background: C.surface, borderRadius: 16, padding: 40, maxWidth: 440, width: "100%", boxShadow: "0 10px 40px rgba(0,0,0,.2)" }}>
        ...
        {listo ? (
          <div style={{ textAlign: "center", padding: "10px 0" }}>
            ...
            <h2 style={{ ...archivo, fontSize: 20, fontWeight: 800, color: C.abyss, marginBottom: 8 }}>ContraseÃ±a actualizada</h2>
            <p style={{ color: C.slate, fontSize: 13.5, lineHeight: 1.6, marginBottom: 20 }}>Ya puedes ingresar con tu nueva contraseÃ±a.</p>
            <button onClick={signOut} style={{ ...primaryBtn, width: "100%" }}>Ir a iniciar sesiÃ³n</button>
          </div>
        ) : (
          <>
            <h2 style={{ ...archivo, fontSize: 22, fontWeight: 800, color: C.abyss, marginBottom: 6 }}>Nueva contraseÃ±a</h2>
            <p style={{ color: C.slate, fontSize: 13.5, marginBottom: 22, lineHeight: 1.5 }}>Define tu nueva contraseÃ±a de acceso al sistema.</p>
            <form onSubmit={submit}>
              <Campo label="Nueva contraseÃ±a">
              ...
              <Campo label="Repetir contraseÃ±a">
                <input type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} required minLength={6} style={{ ...inputStyle(), paddingLeft: 38 }} placeholder="â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢" />
              </Campo>
              ...
              <button type="submit" disabled={busy} style={{ ...primaryBtn, width: "100%", marginTop: 4, opacity: busy ? 0.6 : 1 }}>
                <ShieldCheck size={17} /> {busy ? "Guardandoâ€¦" : "Guardar contraseÃ±a"}
              </button>
            </form>
          </>
        )}
