...
export default function Almacen() {
  ...
  const [ocInit, setOcInit] = useState(null); // Ã­tem pre-cargado al navegar desde "Reponer"
  ...
  const cargar = useCallback(async () => {
    ...
    } catch (e) { setError("No se pudo cargar el almacÃ©n. " + e.message); }
    finally { setLoading(false); }
  }, []);
  useEffect(() => { cargar(); }, [cargar]);

  // Mapa de stock para lecturas rÃ¡pidas: { item_id__bodega_id : cantidad }
  ...
  const itemDesc   = (id) => items.find((i) => i.id === id)?.descripcion || "â€”";
  const itemPrecio = (id) => items.find((i) => i.id === id)?.precio || 0;
  const whName     = (id) => bodegas.find((b) => b.id === id)?.nombre || "â€”";
  ...
  if (loading) return <div><PageHead kicker="GestiÃ³n de Almacenes" title="AlmacÃ©n & Compras" /><Card><InlineSpinner label="Cargando almacÃ©nâ€¦" /></Card></div>;

  return (
    <div>
      <PageHead kicker="GestiÃ³n de Almacenes Â· Libbrecht / Pascual" title="AlmacÃ©n & Compras"
        sub="Bodegas mÃºltiples (tierra + a bordo), movimientos con consumo ligado a OT, y Ã³rdenes de compra con recepciÃ³n que actualiza el stock automÃ¡ticamente." />
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
        ...
        <TabBtn active={tab === "compras"} onClick={() => setTab("compras")} icon={ShoppingCart}>Ã“rdenes de Compra</TabBtn>
      </div>
...
function TabBodegas({ profile, empresa, embarcaciones, bodegas, setBodegas, recargar, setError }) {
  ...
  async function crear() {
    ...
    try {
      ...
      logActivity(profile, "Crear bodega", `${nueva.codigo} Â· ${nueva.nombre}`);
      setForm({ codigo: "", nombre: "", tipo: "tierra", embarcacion_id: "" }); setShowForm(false);
    } catch (e) {
      setError(e.message.includes("duplicate") ? "Ya existe una bodega con ese cÃ³digo." : "No se pudo crear: " + e.message);
    }
  }
  async function eliminar(id) {
    const b = bodegas.find((x) => x.id === id);
    if (!window.confirm(`Â¿Eliminar "${b?.nombre}"? Se borrarÃ¡ tambiÃ©n todo el stock que tenga.`)) return;
    const respaldo = bodegas;
    setBodegas((p) => p.filter((x) => x.id !== id));
    try { await deleteRow("bodegas", id); logActivity(profile, "Eliminar bodega", `${b?.codigo} Â· ${b?.nombre}`); recargar(); }
  ...
  async function autoCrear() {
    ...
    const lista = [{ codigo: "BOD-TIERRA", nombre: `Bodega ${puerto}`, tipo: "tierra", embarcacion_id: null },
      ...embarcaciones.map((e) => ({ codigo: `BOD-${e.codigo}`, nombre: `PaÃ±ol ${e.nombre}`, tipo: "a_bordo", embarcacion_id: e.id }))];
    try {
    ...
    <div>
      ...
      {showForm && (
        <Card style={{ marginBottom: 16, background: C.mist }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
            <Field label="CÃ³digo"><input value={form.codigo} onChange={(e) => setForm({ ...form, codigo: e.target.value })} style={inputStyle()} placeholder="BOD-TIERRA" /></Field>
      ...
      {bodegas.length === 0 ? (
        <Card><Empty>
          ...
          AÃºn no hay bodegas. {embarcaciones.length === 0
            ? "Primero registra al menos una embarcaciÃ³n, luego puedes crear bodegas por defecto automÃ¡ticamente."
            : "Usa \"Crear por defecto\" para generar la bodega de tierra + un paÃ±ol por cada nave, o crea una manualmente."}
        </Empty></Card>
      ) : (
...
function TabStock({ profile, items, setItems, bodegas, stockMap, stock, setStock, setError, onReponer }) {
  ...
  // â”€â”€ DescripciÃ³n â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  async function confirmarDesc(id) {
    ...
    if (!nuevo) { setError("La descripciÃ³n no puede quedar vacÃ­a."); return; }
  ...
  // ABC calculado localmente con los datos disponibles en AlmacÃ©n
  const conABC = (() => {
  ...
  if (bodegas.length === 0) return <Card><Empty><AlertCircle size={28} color={C.amber} /><br/>Primero crea bodegas en la pestaÃ±a <strong>Bodegas</strong>.</Empty></Card>;
  if (items.length === 0)   return <Card><Empty><AlertCircle size={28} color={C.amber} /><br/>Primero carga Ã­tems en <strong>Inventario</strong>.</Empty></Card>;
  ...
  return (
    <div>
      ...
      {/* â”€â”€ Filtros y buscador â”€â”€ */}
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
        <input value={busqueda} onChange={(e) => setBusqueda(e.target.value)}
          placeholder="Buscar cÃ³digo, descripciÃ³n o categorÃ­aâ€¦"
          style={{ ...inputStyle(260), fontSize: 13 }} />
        {[["all","Todos"], ["bajo","Bajo mÃ­nimo"], ["revisar","Por revisar"], ["ok","OK"]].map(([v, lbl]) => {
        ...
        <span style={{ marginLeft: "auto", fontSize: 12, color: C.slate }}>
          {itemsFiltrados.length} de {items.length} Ã­tems
        </span>
      </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 860 }}>
            ...
            <tbody>
              {itemsFiltrados.length === 0 && (
                <tr><td colSpan={4 + bodegas.length} style={{ textAlign: "center", padding: 24, color: C.slate, fontSize: 13 }}>Sin Ã­tems para los filtros seleccionados.</td></tr>
              )}
              {itemsFiltrados.map((i) => {
                ...
                return (
                  <tr key={i.id}>
                    ...
                    <td style={{ ...tdStyle, fontSize: 12, color: C.slate }}>{i.categoria || <span style={{ opacity: 0.35 }}>â€”</span>}</td>
                    ...
                    <td style={{ ...tdStyle, fontSize: 12.5 }}>
                      {descEdit.id === i.id ? (
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          ...
                          {puedeOperar && <button onClick={() => iniciarEditDesc(i.id, i.descripcion)} title="Editar descripciÃ³n" style={btnEdit}><Pencil size={12} /></button>}
                        </div>
                      )}
                    </td>

                    {/* Celdas de stock por bodega con confirmaciÃ³n */}
                    {bodegas.map((b) => {
                    ...
                    <td style={{ ...tdStyle, minWidth: 140 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 5 }}>
                        ...
                        {puedeOperar && (
                          <button onClick={() => onReponer(i)} title="Crear OC para reponer este Ã­tem"
...
function TabMovimientos({ profile, items, bodegas, ots, movimientos, stockMap, itemDesc, whName, recargar, setError }) {
  ...
  async function registrar() {
    if (!form.item_id || form.cantidad <= 0) { setError("Selecciona el Ã­tem y una cantidad mayor a 0."); return; }
    const cant = Number(form.cantidad);
    try {
      // 1) Actualizar stock primero (lo mÃ¡s crÃ­tico)
      ...
      logActivity(profile, `Movimiento: ${form.tipo}`, `${cant}Ã— ${itemDesc(form.item_id)}${form.ot_id ? " Â· OT" : ""}`);
  ...
  function exportar() {
    const filas = [["Fecha", "Tipo", "Ãtem", "Cantidad", "Origen", "Destino", "OT", "Responsable", "Motivo"],
  ...
  if (items.length === 0 || bodegas.length === 0) {
    return <Card><Empty><AlertCircle size={28} color={C.amber} /><br/>Necesitas Ã­tems en Inventario y al menos una bodega para registrar movimientos.</Empty></Card>;
  }
      ...
      {puedeOperar && (
        <Card style={{ marginBottom: 16, background: C.mist }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
            ...
            <Field label="Ãtem" span={2}><select value={form.item_id} onChange={(e) => setForm({ ...form, item_id: e.target.value })} style={inputStyle()}>
              <option value="">â€” Selecciona â€”</option>
              {items.map((i) => <option key={i.id} value={i.id}>{i.codigo} Â· {i.descripcion}</option>)}
            </select></Field>
            ...
            {form.tipo === "salida" && (
              <Field label="OT asociada (opcional)"><select value={form.ot_id} onChange={(e) => setForm({ ...form, ot_id: e.target.value })} style={inputStyle()}>
                ...
                {ots.map((o) => <option key={o.id} value={o.id}>{o.folio} Â· {o.sistema}</option>)}
              </select></Field>
            )}
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 920 }}>
            <thead><tr>
              <th style={thStyle}>Fecha</th><th style={thStyle}>Tipo</th><th style={thStyle}>Ãtem</th>
              <th style={{ ...thStyle, textAlign: "right" }}>Cant.</th><th style={thStyle}>Origen â†’ Destino</th>
              ...
              {movimientos.length === 0 ? <tr><td colSpan={7}><Empty>Sin movimientos registrados.</Empty></td></tr> :
                movimientos.map((m) => (
                  <tr key={m.id}>
                    ...
                    <td style={{ ...tdStyle, fontSize: 12 }}>{m.bodega_from ? whName(m.bodega_from) : "â€”"} <ChevronRight size={11} style={{ display: "inline", verticalAlign: "middle" }} /> {m.bodega_to ? whName(m.bodega_to) : "â€”"}</td>
...
function TabCompras({ profile, items, bodegas, compras, comprasItems, stockMap, itemDesc, itemPrecio, whName, ocInit, onOcInitUsed, recargar, setError }) {
  ...
  // Inicializar form desde botÃ³n "Reponer" en Stock
  React.useEffect(() => {
  ...
  // Sugerencias de reposiciÃ³n: Ã­tems bajo mÃ­nimo (total)
  ...
  // â”€â”€ RecepciÃ³n parcial â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    try {
      ...
      for (const it of its) {
        ...
        await insertRow("movimientos", profile.empresa_id, {
          ...
          motivo: `RecepciÃ³n ${todoRecibido ? "completa" : "parcial"} ${oc.folio}`, created_by: profile.id,
        });
      ...
      if (todoRecibido) {
        ...
        logActivity(profile, "Recibir OC completa", `${oc.folio} â†’ ${whName(oc.bodega_destino)}`);
      } else {
        logActivity(profile, "RecepciÃ³n parcial OC", `${oc.folio} â€” pendiente de completar`);
      }
      setRecepPanel(null); setRecepCants({});
      recargar();
    } catch (e) { setError("No se pudo procesar la recepciÃ³n: " + e.message); }
  }

  async function crearOC() {
    if (!form.proveedor.trim() || form.items.length === 0) { setError("Indica proveedor y al menos un Ã­tem."); return; }
    ...
    try {
      ...
      logActivity(profile, "Crear OC", `${folio} Â· ${form.proveedor} Â· ${form.items.length} Ã­tems`);
    ...
    try {
      if (next === "recibida") {
        ...
        for (const it of ocItemsList(oc)) {
          ...
          await insertRow("movimientos", profile.empresa_id, {
            fecha: HOY(), tipo: "entrada", item_id: it.item_id, bodega_to: oc.bodega_destino,
            cantidad: it.cantidad, responsable: "Compras", motivo: `RecepciÃ³n ${oc.folio}`, created_by: profile.id,
          });
        }
        await updateRow("compras", oc.id, { estado: "recibida", fecha_recepcion: HOY() });
        logActivity(profile, "Recibir OC", `${oc.folio} Â· stock ingresado a ${whName(oc.bodega_destino)}`);
      } else {
        await updateRow("compras", oc.id, { estado: next });
        logActivity(profile, "Avanzar OC", `${oc.folio} â†’ ${next}`);
      }
      recargar();
  ...
  async function eliminar(id) {
    ...
    if (!window.confirm(`Â¿Eliminar la orden ${o?.folio}?`)) return;
  ...
  if (items.length === 0 || bodegas.length === 0) {
    return <Card><Empty><AlertCircle size={28} color={C.amber} /><br/>Necesitas Ã­tems en Inventario y al menos una bodega para crear compras.</Empty></Card>;
  }
    ...
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 16 }}>
        ...
        <Card style={{ padding: 14 }}>
          ...
          <div style={{ fontSize: 11, color: C.slate, marginTop: 2 }}>no recibido aÃºn</div>
        </Card>
        <Card style={{ padding: 14 }}>
          <div style={{ fontSize: 11, letterSpacing: 1, textTransform: "uppercase", color: C.slate, fontWeight: 600 }}>Ãtems Bajo MÃ­nimo</div>
          <div style={{ ...archivo, fontSize: 24, fontWeight: 800, color: sugerencias.length ? C.red : C.green, marginTop: 6 }}>{sugerencias.length}</div>
          <div style={{ fontSize: 11, color: C.slate, marginTop: 2 }}>requieren reposiciÃ³n</div>
        </Card>
      </div>
        ...
        <Card style={{ marginBottom: 16, border: `1px solid ${C.amber}`, background: C.yellowBg }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <strong style={{ color: C.abyss, fontSize: 14 }}>Sugerencias de ReposiciÃ³n</strong>
            </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {sugerencias.map((s) => (
              <div key={s.id} style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: 7, padding: "6px 10px", fontSize: 12 }}>
                <strong>{s.descripcion}</strong> <span style={{ color: C.slate }}>Â· {s.total}/{s.stock_min}</span> <span style={{ fontFamily: "'IBM Plex Mono', monospace", color: C.green, fontWeight: 600 }}>â†’ {s.sugerido}</span>
              </div>))}
          </div>
        </Card>
      )}
      ...
      {showForm && (
        <Card style={{ marginBottom: 16, background: C.mist }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 2fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
            ...
            <Field label="NÂ° ref. proveedor"><input value={form.ref_proveedor} onChange={(e) => setForm({ ...form, ref_proveedor: e.target.value })} style={inputStyle()} placeholder="OC-PROV-001" /></Field>
          </div>
          <div style={{ marginBottom: 12 }}>
            <Field label="Notas u observaciones"><input value={form.notas} onChange={(e) => setForm({ ...form, notas: e.target.value })} style={{ ...inputStyle(), width: "100%" }} placeholder="Instrucciones de entrega, contacto, urgenciaâ€¦" /></Field>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "3fr 1fr auto", gap: 12, alignItems: "flex-end", marginBottom: 12 }}>
            <Field label="Ãtem"><select value={line.item_id} onChange={(e) => setLine({ ...line, item_id: e.target.value })} style={inputStyle()}>
              <option value="">â€” Selecciona â€”</option>
              {items.map((i) => <option key={i.id} value={i.id}>{i.codigo} Â· {i.descripcion}</option>)}
            </select></Field>
              ...
              {form.items.map((it, idx) => (
                <div key={idx} style={{ display: "flex", justifyContent: "space-between", padding: "5px 6px", borderBottom: idx < form.items.length - 1 ? `1px solid ${C.foam}` : "none", fontSize: 12.5 }}>
                  <span>
                    ...
                    {" Â· "}{itemDesc(it.item_id)}
                    {itemCategoria(it.item_id) && <span style={{ color: C.slate, fontSize: 11.5 }}> [{itemCategoria(it.item_id)}]</span>}
                    {" Â· "}<span style={{ fontFamily: "'IBM Plex Mono', monospace" }}>{it.cantidad} Ã— {clp(it.precio)}</span>
                  </span>
      ...
      {/* â”€â”€ Panel de recepciÃ³n parcial â”€â”€ */}
      {recepPanel && (() => {
        ...
        return (
          <Card style={{ marginBottom: 16, borderLeft: `4px solid ${C.cyan}`, background: "#F0FAFF" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              <div>
                <div style={{ fontSize: 10, letterSpacing: 2, textTransform: "uppercase", color: C.slate, fontWeight: 700 }}>RecepciÃ³n de mercaderÃ­a</div>
                <div style={{ fontWeight: 800, fontSize: 15, color: C.abyss, marginTop: 2 }}>
                  {oc.folio} Â· {oc.proveedor} â€” indica las cantidades que efectivamente llegaron
                </div>
              </div>
                  ...
                  return (
                    <tr key={it.id}>
                      ...
                      <td style={{ ...tdStyle, textAlign: "center" }}>
                        ...
                        ) : <span style={{ fontSize: 12, color: C.green }}>âœ“ Completo</span>}
                      </td>
                    </tr>
                  );
                })}
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => confirmarRecepcion(oc)} style={primaryBtn}><Check size={15} /> Confirmar recepciÃ³n</button>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 920 }}>
            <thead><tr>
              ...
              <th style={thStyle}>Ãtems</th><th style={thStyle}>Destino</th>
              <th style={{ ...thStyle, textAlign: "right" }}>Total</th>
              <th style={thStyle}>ETA</th><th style={thStyle}>Ref / Notas</th>
              <th style={thStyle}>Estado</th><th style={thStyle}>AcciÃ³n</th>{puedeBorrar && <th style={thStyle}></th>}
            </tr></thead>
            <tbody>
              {compras.length === 0 ? <tr><td colSpan={puedeBorrar ? 10 : 9}><Empty>Sin Ã³rdenes de compra.</Empty></td></tr> :
                compras.map((o) => {
                  ...
                  return (
                    <tr key={o.id}>
                      ...
                      <td style={{ ...tdStyle, maxWidth: 260 }}>
                        {its.map((it, idx) => (
                          <div key={idx} style={{ fontSize: 12, lineHeight: 1.5 }}>
                            ...
                            <span style={{ color: C.ink }}> {it.cantidad}Ã— {itemDesc(it.item_id)}</span>
                      ...
                      <td style={tdStyle}>
                        {(() => { const s = etaSemaforo(o); return s ? (
                          <div>
                            ...
                            <div style={{ fontSize: 11, fontWeight: 700, color: s.color, marginTop: 2 }}>â— {s.label}</div>
                          </div>
                        ) : o.estado === "recibida" ? <span style={{ fontSize: 11, color: C.green }}>âœ“ Recibida</span> : <span style={{ fontSize: 11, color: C.slate }}>â€”</span>; })()}
                      </td>
                      <td style={{ ...tdStyle, maxWidth: 180 }}>
                        ...
                        {!o.ref_proveedor && !o.notas && <span style={{ color: C.line }}>â€”</span>}
                      </td>
                      ...
                      <td style={tdStyle}>{o.estado !== "recibida" && puedeAprobar ? (
                        o.estado === "enviada" ? (
                          ...
                          <button onClick={() => avanzar(o)} style={{ ...ghostBtn, padding: "5px 10px", fontSize: 12 }}>
                            {o.estado === "solicitada" ? "Aprobar" : "Enviar â†’"}
                          </button>
                        )
...
/* â”€â”€ Timeline visual de estado OC â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
const OC_STEPS = [
...
/* â”€â”€ Barra visual de nivel de stock â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
  ...
  return (
    <div style={{ minWidth: 110 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9.5, marginTop: 2 }}>
        ...
        <span style={{ color: C.slate, opacity: 0.6 }}>mÃ¡x {cap}</span>
      </div>
    </div>
  );
}
