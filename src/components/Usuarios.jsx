...
export default function Usuarios() {
  ...
  async function setCampo(u, campo, valor) {
    ...
    try {
      ...
      logActivity(profile, "GestiÃ³n de usuario", `${u.nombre || u.email}: ${campo} â†’ ${valor}`);
      if (u.id === profile.id) refreshProfile(); // si me editÃ© a mÃ­ mismo, refresco mi sesiÃ³n
    } catch (e) {
  ...
  async function eliminar(u) {
    ...
    if (!window.confirm(`Â¿Quitar a "${u.nombre || u.email}" de la empresa? PerderÃ¡ el acceso a los datos (su cuenta de correo seguirÃ¡ existiendo, pero sin empresa asignada).`)) return;
  ...
  if (loading) return <div><PageHead kicker="AdministraciÃ³n" title="GestiÃ³n de Usuarios" /><Card><InlineSpinner label="Cargando usuariosâ€¦" /></Card></div>;

  if (!soyAdmin) {
    return (
      <div>
        <PageHead kicker="AdministraciÃ³n" title="GestiÃ³n de Usuarios" />
        <Card><Empty>
          <Shield size={32} color={C.amber} style={{ marginBottom: 10 }} /><br />
          Esta secciÃ³n es solo para administradores. Si necesitas cambios en tu cuenta, contacta al Jefe de MantenciÃ³n o al administrador de tu empresa.
        </Empty></Card>
      </div>
    );
  }
  ...
  return (
    <div>
      <PageHead kicker="AdministraciÃ³n Â· Equipo" title="GestiÃ³n de Usuarios"
        sub="Administra quiÃ©n accede al sistema, con quÃ© rol y a quÃ© embarcaciÃ³n. Los nuevos empleados se registran con el cÃ³digo de la empresa y aparecen aquÃ­."
      ...
      <Card style={{ marginBottom: 16, padding: 20, background: `linear-gradient(135deg, ${C.abyss}, ${C.steel})`, color: "#fff" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
          <div>
            <div style={{ fontSize: 11, letterSpacing: 1.5, textTransform: "uppercase", color: "rgba(255,255,255,.7)", fontWeight: 700, marginBottom: 6 }}>CÃ³digo de invitaciÃ³n de {empresa?.nombre}</div>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,.85)", maxWidth: 460, lineHeight: 1.5 }}>
              Comparte este cÃ³digo con tus empleados. Al registrarse e ingresarlo, quedan asignados automÃ¡ticamente a tu empresa (como maquinista). Luego les ajustas el rol aquÃ­.
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ ...archivo, fontSize: 30, fontWeight: 800, letterSpacing: 3, color: C.gold, fontFamily: "'IBM Plex Mono', monospace" }}>
              {empresa?.codigo_invitacion || "â€”"}
            </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 16 }}>
        ...
        <KPI label="Administradores" value={admins} tone={C.steel} sub="acceso de gestiÃ³n" />
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 920 }}>
            <thead><tr>
              ...
              <th style={thStyle}>Rol</th><th style={thStyle}>EmbarcaciÃ³n</th>
              <th style={{ ...thStyle, textAlign: "center" }}>Activo</th><th style={thStyle}></th>
            </tr></thead>
            <tbody>
              {usuarios.length === 0 ? <tr><td colSpan={6}><Empty>Sin usuarios. Comparte el cÃ³digo de invitaciÃ³n para que tu equipo se registre.</Empty></td></tr> :
                usuarios.map((u) => {
                  ...
                  return (
                    <tr key={u.id} style={{ background: soyYo ? C.mist : "transparent" }}>
                      <td style={tdStyle}>
                        ...
                        {soyYo && <span style={{ fontSize: 10.5, color: C.steel, fontWeight: 600, marginLeft: 6 }}>(tÃº)</span>}
                      </td>
                      <td style={{ ...tdStyle, fontSize: 12, color: C.slate, fontFamily: "'IBM Plex Mono', monospace" }}>{u.email || "â€”"}</td>
                      <td style={tdStyle}>
                        {puedoEditarRol ? (
                      ...
                      <td style={tdStyle}>
                        <select value={u.embarcacion_id || ""} onChange={(e) => setCampo(u, "embarcacion_id", e.target.value || null)} style={inputStyle(150)}>
                          <option value="">â€” Toda la flota â€”</option>
      ...
      <Card style={{ marginTop: 16, background: C.mist }}>
        <div style={{ fontSize: 12.5, color: C.slate, lineHeight: 1.7 }}>
          <strong style={{ color: C.ink }}>CÃ³mo dar de alta a alguien:</strong> 1) le pasas el <strong>cÃ³digo de invitaciÃ³n</strong> de arriba;
          2) la persona entra a la app, pulsa <strong>"Crear una"</strong> cuenta y lo ingresa al registrarse;
          3) confirma su correo e inicia sesiÃ³n â€” ya queda en tu empresa como maquinista;
          4) tÃº le ajustas aquÃ­ el rol y la embarcaciÃ³n.
