      ...
      {showForm && (
        <Card style={{ marginBottom: 16, background: C.mist }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
            ...
            <Field label="Severidad (1-10)"><input type="number" min={1} max={10} value={form.severidad} onFocus={(e) => e.target.select()} onChange={(e) => setForm({ ...form, severidad: Math.max(1, Math.min(10, +e.target.value)) })} style={bluInput} /></Field>
            <Field label="Ocurrencia (1-10)"><input type="number" min={1} max={10} value={form.ocurrencia} onFocus={(e) => e.target.select()} onChange={(e) => setForm({ ...form, ocurrencia: Math.max(1, Math.min(10, +e.target.value)) })} style={bluInput} /></Field>
            <Field label="Detección (1-10)"><input type="number" min={1} max={10} value={form.deteccion} onFocus={(e) => e.target.select()} onChange={(e) => setForm({ ...form, deteccion: Math.max(1, Math.min(10, +e.target.value)) })} style={bluInput} /></Field>
                ...
                lista.map((f) => {
                  ...
                  return (
                    <tr key={f.id}>
                      ...
                      <td style={{ ...tdStyle, textAlign: "center" }}><input type="number" min={1} max={10} value={f.severidad} disabled={!puedeOperar} onFocus={(e) => e.target.select()} onChange={(e) => onChangeLocal(f.id, "severidad", +e.target.value)} onBlur={(e) => commit(f.id, "severidad", Math.max(1, Math.min(10, +e.target.value)))} style={{ ...bluInput, width: 50, textAlign: "center" }} /></td>
                      <td style={{ ...tdStyle, textAlign: "center" }}><input type="number" min={1} max={10} value={f.ocurrencia} disabled={!puedeOperar} onFocus={(e) => e.target.select()} onChange={(e) => onChangeLocal(f.id, "ocurrencia", +e.target.value)} onBlur={(e) => commit(f.id, "ocurrencia", Math.max(1, Math.min(10, +e.target.value)))} style={{ ...bluInput, width: 50, textAlign: "center" }} /></td>
                      <td style={{ ...tdStyle, textAlign: "center" }}><input type="number" min={1} max={10} value={f.deteccion} disabled={!puedeOperar} onFocus={(e) => e.target.select()} onChange={(e) => onChangeLocal(f.id, "deteccion", +e.target.value)} onBlur={(e) => commit(f.id, "deteccion", Math.max(1, Math.min(10, +e.target.value)))} style={{ ...bluInput, width: 50, textAlign: "center" }} /></td>
